...
* Present the user with a textbox where they can list comma-separated items they usually eat for lunch. 
* Once that's entered, the user has to click the "Check If Too Much" button.
* If the number of items in the textbox is less than or equal to 3, show a message under the textbox saying "Enjoy!"
* If the number of items is greater than 3, show a message under the textbox saying "Too much!"
* If the textbox is empty and the user clicks the "Check If Too Much" button, the message "Please enter data first" should show up.
  * 'Empty' means either "" (empty string) or a string with just spaces in it.
